...
class Mage_Page_Block_Html_Topmenu extends Mage_Core_Block_Template
{
    ...
    /**
     * Get top menu select html
     *
     * @param string $outermostClass
     * @param string $childrenWrapClass
     * @return string
     */
    public function getSelectHtml($outermostClass = '', $childrenWrapClass = '')
    {
        Mage::dispatchEvent('page_block_html_topmenu_getselecthtml_before', array(
            'menu' => $this->_menu,
            'block' => $this
        ));
        ...
        $html = $this->_getSelectHtml($this->_menu, $childrenWrapClass);

        Mage::dispatchEvent('page_block_html_topmenu_getselecthtml_after', array(
            'menu' => $this->_menu,
            'html' => $html
        ));

        return '<select class="'.$outermostClass.'"><option>Navigate</option>'.$html.'</select>';
    }
...
//        }
    ...
    /**
     * Recursively generates top menu html from data that is specified in $menuTree into a select dom
     *
     * @param Varien_Data_Tree_Node $menuTree
     * @param string $childrenWrapClass
     * @see Varien_Data_Tree_Node
     * @return string
     */
    protected function _getSelectHtml(Varien_Data_Tree_Node $menuTree, $childrenWrapClass)
    {
        $_html = array();
        $html = '';
        $children = $menuTree->getChildren();
...
//        }
        ...
        foreach ($children as $child) {
            ...
            if (!$child->hasChildren()) {
                $html .= '<option' . ($liInternals = $this->_getRenderedMenuItemAttributes($child)) . " ";
                $html .= 'value="' . $child->getUrl() . '/" ' . $outermostClassCode . '><span>'
                      .  $this->escapeHtml($child->getName()) . '</span></option>';
            } else {
                $html .= '<optgroup label="'.$this->escapeHtml($child->getName()).'">';
                $html .= '<option' . ($liInternals = $this->_getRenderedMenuItemAttributes($child)) . " ";
                $html .= 'value="' . $child->getUrl() . '/" ' . $outermostClassCode . '><span>'
                    .  $this->escapeHtml($child->getName()) . '</span></option>';
                if (!empty($childrenWrapClass)) {
                ...
                $html .= $this->_getSelectHtml($child, $childrenWrapClass);
                $html .= '</optgroup>';
