...
class Mage_Page_Block_Template_Links extends Mage_Core_Block_Template
{
    ...
    /**
     * Update link in the list by URL
     *
     * @param string $url
     * @param string $label
     * @param string $newUrl
     ...
     * @return Mage_Page_Block_Template_Links
     */
    public function updateLinkByUrl($url, $label='', $newUrl='', $title='', $prepare=false, $urlParams=array(),
                                    $position=null, $liParams=null, $aParams=null, $beforeText='', $afterText='')
    {
        if (is_null($url) || false===$url || empty($url)) {
            return $this;
        }

        foreach ($this->_links as $k => $v) {
            if ($v->getUrl() == $url) {
                $v->setLabel(is_null($label) || false===$label || empty($label) ? $v->getLabel() : $label);
                $v->setUrl(is_null($newUrl) || false===$newUrl || empty($newUrl) ? $v->getUrl() : ($prepare ? $this->getUrl($newUrl, (is_array($urlParams) ? $urlParams : array())) : $newUrl));
                $v->setTitle(is_null($title) || false===$title || empty($title) ? $v->getTitle() : $title);
                $v->setLiParams(is_null($liParams) || false===$liParams || empty($liParams) ? $v->getLiParams() : $this->_prepareParams($liParams));
                $v->setAParams(is_null($aParams) || false===$aParams || empty($aParams) ? $v->getAParams() : $this->_prepareParams($aParams));
                $v->setBeforeText(is_null($beforeText) || false===$beforeText || empty($beforeText) ? $v->getBeforeText() : $beforeText);
                $v->setAfterText(is_null($afterText) || false===$afterText || empty($afterText) ? $v->getAfterText() : $afterText);

                if(!(is_null($position) || false===$position || empty($position)) && $k!==$position) {
                    unset($this->_links[$k]);
                    $this->_addIntoPosition($v,$position);
                }
                break;
            }
        }
