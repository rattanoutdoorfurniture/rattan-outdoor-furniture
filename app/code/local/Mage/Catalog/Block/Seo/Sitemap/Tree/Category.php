...
class Mage_Catalog_Block_Seo_Sitemap_Tree_Category extends Mage_Catalog_Block_Seo_Sitemap_Category
{
    ...
    public function prepareCategoriesToPagesCorrectly() {
        $linesPerPage = Mage::getStoreConfig(self::XML_PATH_LINES_PER_PAGE);
        $tmpCollection = Mage::getModel('catalog/category')->getCategories(Mage::app()->getStore()->getRootCategoryId());
        return $this->_prepareCategoriesToPagesCorrectly($tmpCollection, $linesPerPage);

    }

    private function _prepareCategoriesToPagesCorrectly($categories, $lines, $currentPage = 1) {
        foreach($categories as $category) {
            ...
            if(count($this->categoriesToPages[$currentPage])>=$lines) $currentPage++;
            $this->_categoriesToPages[$currentPage][$category->getId()] = array(
                'path'              => $category->getPath(),
                'children_count'    => $childCount
            );
            if($childCount>0) $this->_prepareCategoriesToPagesCorrectly($children,$lines,$currentPage);
        }
        return $this->_categoriesToPages;
    }
